...
def repair_program_io(code):
    ...
    lines = code.split("\n")  # 将代码按行分割
    lines_flags = [0 for _ in range(len(lines))]  # 初始化标志列表
    ...
    if not bool_repaired:  # 如果没有典型的模式，则仅在每个Out之后删除标志为0的行
        repaired_code = ""
        sub_block = ""
                ...
                if lineno > len(code_lines) - 1:
                    ...
                    if posno < len(failed_code_line) - 1:
                        ...
                        tokenized_failed_code_line = wordpunct_tokenize(
                            failed_code_line)  # 用wordpunct_tokenizer分词错误行
